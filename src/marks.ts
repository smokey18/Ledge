...
export function mark(agent: string, label: string): SVGElement | HTMLElement {
  const path = PATHS[agent];
  if (!path) {
    const initial = document.createElement("span");
    initial.className = "mark initial";
  ...
  svg.setAttribute("class", `mark ${agent}`);
  svg.innerHTML = `<path d="${path}" fill-rule="evenodd" clip-rule="evenodd" />`;
  return svg;
}
